...
########################################################
# Graph WebPlotDigitizer data extracted from PHS graph #
########################################################
...
vlines.cardiac.calls <- data.frame(Date = c(as.Date("2019-01-01"),as.Date("2020-01-01"),as.Date("2021-01-01"),as.Date("2022-01-01")),
                                             Year = c(2019,2020,2021,2022), # This adds year labels to the lower faceted graph only
...
cardiac.calls.graph <- ggplot(data = cardiac.calls,
                              aes(x = Date,
                                  y = Calls,
                                  colour = Type,
                                  linetype = Type)
  ...
  scale_x_date(name = "Date",
               #limits = c(as.Date("2020-09-01"),as.Date("2021-12-31")),
               breaks = "1 month",
...
ggsave(cardiac.calls.graph,filename = paste(GraphFileNameRoot,"_CardiacCalls.png",sep=""),
       device = png,
       #dpi = 320,
       units = "px",
       width = 2300,
